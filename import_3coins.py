# -*- coding: utf-8 -*-
"""一次性:把 scrape_3coins.py 抓好的 3coins_full_products.json 匯入資料庫
------------------------------------------------------------
用途:
  scrape_3coins.py 只負責抓資料、輸出 JSON 檔,不會寫入 Firebase(資料庫
...
執行方式(透過 GitHub Actions 手動觸發):
  ...
  "One-off: import 3COINS catalog" -> 右邊 "Run workflow" 按鈕
"""
...
def main():
    ...
    with open("3coins_full_products.json", encoding="utf-8") as f:
    ...
    existing_links = {p.get("link") for p in products if p.get("brand") == "3COINS"}
    print(f"資料庫裡已經有 {len(existing_links)} 件 3COINS 商品")
