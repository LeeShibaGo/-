...
#
# 這份清單曾經漏掉「深綠(ダークグリーン)」「淺藍(ライトブルー)」「酒紅
# (バーガンディー)」這幾個具體色名,結果它們被排序在後面的泛用規則
# (グリーン→綠色、ブルー→藍色)搶先命中,導致同一件商品的深綠/淺綠、
# 深藍/淺藍兩種顏色都被翻成一模一樣的中文,看起來像沒得選一樣。
# 所以順序原則是:越具體/越長的詞要排在越前面。
COLOR_JA_TO_ZH = [
    ("ライトグレー", "淺灰"),
    ("チャコール", "炭灰色"),
    ("オフホワイト", "米白"),
...
# AAPE 自己的顏色命名裡,同一個色系常常有好幾個「沒有專屬名字」的變化色,
# 官網就直接標成「グリーン系その他」「グリーン系その他2」「グリーン系その他6」
# 這種「底色 + 系その他 + 編號」格式。以前只翻底色,結果「其他」跟「其他2」
# 兩個明明是不同顏色,卻被翻成一模一樣的中文。這裡把編號一起保留下來,
# 至少客人看得出來這是「同色系的另一款」,不會誤以為是同一個顏色。
SERIES_OTHER_RE = re.compile(r"^(.*?)系その他(\d*)$")


def translate_color(name):
    name = name.strip()

    # 「黑×紫」這種雙色/撞色款,兩邊分開翻譯再組回去
    if "×" in name:
        return "×".join(translate_color(part) for part in name.split("×"))

    m = SERIES_OTHER_RE.match(name)
    if m:
        base, suffix = m.group(1), m.group(2)
        base_zh = translate_color(base)
        return f"{base_zh}(其他{suffix})"
