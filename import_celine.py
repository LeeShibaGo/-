# -*- coding: utf-8 -*-
"""一次性:把 scrape_celine.py 抓好的 celine_all_products.json 匯入資料庫
------------------------------------------------------------
用途:
  用服務帳號(Admin SDK)把那份 JSON 加上商品 id 之後,合併進現有的
  daigou-products-v1(不影響其他品牌的資料)。用 link 當比對鍵,已經
  存在的商品會被跳過,重跑這支是安全的。

執行方式(透過 GitHub Actions 手動觸發):
  GitHub 網頁 -> 這個 repo -> Actions 分頁 -> 左邊選
  "One-off: import CELINE catalog" -> 右邊 "Run workflow" 按鈕
"""
...
def main():
    ...
    with open("celine_all_products.json", encoding="utf-8") as f:
        fresh_items = json.load(f)
    print(f"待匯入商品共 {len(fresh_items)} 款")

    products = db.reference(PRODUCTS_PATH).get()
    if isinstance(products, dict):
        products = list(products.values())
    products = [p for p in products if p]
    existing_links = {p.get("link") for p in products if p.get("brand") == "CELINE"}
    print(f"資料庫裡已經有 {len(existing_links)} 件 CELINE 商品")
    ...
    for idx, item in enumerate(fresh_items):
        if item.get("link") in existing_links:
            continue
        item["id"] = f"p_celine_{ts}_{idx}"
        item["addedAt"] = ts
        products.append(item)
        added += 1

    print(f"新增 {added} 款(其餘已存在,略過)")
