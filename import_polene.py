# -*- coding: utf-8 -*-
"""一次性:把 scrape_polene.py 抓好的 polene_all_products.json 匯入資料庫
------------------------------------------------------------
用途:
  scrape_polene.py 只負責抓資料、輸出 JSON 檔,不會寫入 Firebase(資料庫
...
執行方式(透過 GitHub Actions 手動觸發):
  ...
  "One-off: import POLENE catalog" -> 右邊 "Run workflow" 按鈕
"""
...
def main():
    ...
    with open("polene_all_products.json", encoding="utf-8") as f:
    ...
    existing_links = {p.get("link") for p in products if p.get("brand") == "POLENE"}
    print(f"資料庫裡已經有 {len(existing_links)} 件 POLENE 商品")
    ...
    # 2026-08-14 起前端首頁改成先抓精簡索引 daigou-products-index-v1,
    # 見 sync_stock.py 開頭的說明——這裡也要跟著更新,不然新匯入的商品
    # 在索引重建前,首頁的品牌導覽/搜尋會看不到。
